...
def node_dictionaries(
    ...
    _, close_pairs = simetri.geom.polygons.polygon.all_close_points(
...
def symmetric_difference(shapes, dist_tol, debug=False):
    ...
    for x in ips_edges:
        ip, edges = x
        ip = simetri.geom.points.point_utils.round_point(
            ip, 2
        )  # is this necessary?
...
def tests():
    ...
    for n in [10, 20, 40, 80, 160, 320, 640, 1280, 2560]:
        ...
        while count < n:
            ...
            p2 = rp()
            edge = (p1, p2)
            if simetri.geom.points.point_utils.distance(p1, p2) < 40:
                edges.append(edge)
                count += 1
...
def line_sweep(segments):
    ...
    for s1, s2 in intersections:
        ip = simetri.geom.segments.line_utils.intersect2(
