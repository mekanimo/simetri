...
def get_partitions_old(shapes, length_bound=10):
    ...
    union_area = simetri.geom.polygons.polygon.polygon_area(union.vertices)
...
def get_partitions(shapes, length_bound=10):
    ...
    for edge_id, edge in enumerate(shapes.all_segments):
        ...
        points_by_edge[edge_id].extend(
            (
                simetri.geom.points.point_utils.round_point(start_point),
                simetri.geom.points.point_utils.round_point(end_point),
            )
        )
    for point, (first_edge_id, second_edge_id) in intersections:
        rounded_point = simetri.geom.points.point_utils.round_point(point)
    ...
    for i in range(n_rows):
        ...
        if len(points) >= 2:
            ...
            all_midpoints.extend(
                simetri.geom.points.point_utils.midpoint(*segment)
    ...
    for poly in cycles:
        ...
        if not simetri.geom.polygons.polygon.any_point_inside_polygon(
