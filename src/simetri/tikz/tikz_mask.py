...
def _normalize_mask_inputs(mask, **kwargs):
    ...
    if isinstance(mask, Mask) or (
        hasattr(mask, "shape")
        and hasattr(mask, "opacity")
        and hasattr(mask, "stops")
    ):
    ...
    if mask_stops is not None:
        if not isinstance(mask_stops, (list, tuple)) or len(mask_stops) < 2:
            raise ValueError(
                "mask stops must be a list with at least two SVG stop entries."
            )
        for coord_name, coord in (
            ...
            if not isinstance(coord, (int, float)):
                raise TypeError(
                    f"mask {coord_name} must be a float between 0 and 1."
                )
...
def clip_mask(
    ...
    **kwargs,
    ...
    if target is None:
        ...
        if use_gradient:
            fade_id = f"simetriCanvasMaskFade{scope_sketch.id}"
            fade_code = _build_fading_code(
                fade_id, mask_stops, mask_x1, mask_y1, mask_x2, mask_y2
            )
    ...
    if not clip_code:
        raise ValueError(
            "TikZ masking currently requires a clip-compatible mask shape."
        )
    ...
    if target.type == Types.GROUP:
        for item in target:
            self.active_page.sketches.extend(
                canvas_draw.get_sketches(item, self, **kwargs)
            )
    else:
        self.active_page.sketches.extend(
            canvas_draw.get_sketches(target, self, **kwargs)
        )
    self._all_vertices.extend(mask_shape.b_box.corners)
