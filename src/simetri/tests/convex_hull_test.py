...
hull = convex_hull(squares.all_vertices)
hull_shape = sg.Shape(
    hull, closed=True, fill=False, line_color=sg.red, line_width=2
)
canvas = sg.Canvas()

canvas.draw([hull_shape, squares], fill=False)

canvas.save("c:/tmp/convex_hull_test.svg", overwrite=True)
